...
use super::upstream_support_and_manual_jobs::*;
    ...
    #[tokio::test]
    async fn body_gc_index_ensure_requeues_after_a_transient_build_failure() {
        let db_path = temp_db_path("maintenance-worker-body-gc-index-retry");
        let db_str = db_path.to_string_lossy().to_string();
        let proxy = TavilyProxy::with_endpoint(Vec::<String>::new(), DEFAULT_UPSTREAM, &db_str)
            .await
            .expect("proxy created");
        ...
        sqlx::query("CREATE TABLE observability.idx_request_logs_body_gc_cursor (id INTEGER)")
            .execute(&pool)
            .await
            .expect("reserve the index name to force a transient build failure");
        let job_id = state
            .proxy
            .scheduled_job_claim(
            ...
            .await
            .expect("claim body GC index job")
            .expect("body GC index job created");

        assert!(
            !run_request_logs_body_gc_index_ensure_claimed_job(
                state.clone(),
                ClaimedScheduledJob {
                    job_id,
        ...
        let jobs: Vec<(i64, String, String, i64)> = sqlx::query_as(
            "SELECT id, status, trigger_source, available_at FROM scheduled_jobs ORDER BY id ASC",
        )
        .fetch_all(&pool)
        .await
        ...
        assert!(
            jobs[1].3
                >= state.proxy.backend_time().now_ts()
                    + REQUEST_LOGS_BODY_GC_INDEX_ENSURE_RETRY_DELAY_SECS
                    - 1
        );
