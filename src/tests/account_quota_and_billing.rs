...
async fn list_recent_jobs_paginated_includes_key_group() {
    ...
    proxy
        .scheduled_job_finish(linuxdo_tag_refresh_job_id, "success", Some("refreshed=4"))
        .await
    ...
    assert!(
        logs_items
            .iter()
            .any(|item| item.job_type == "mcp_sessions_gc")
    );
    assert!(
        logs_items
            .iter()
    ...
    assert!(
        linuxdo_items.iter().any(
            |item| item.job_type == "linuxdo_user_tag_binding_refresh" && item.key_id.is_none()
        )
    );
...
async fn monthly_quota_rebase_startup_gate_runs_once_and_manual_rebase_remains_idempotent() {
    ...
    proxy
        ...
        .flush_request_stats_writes()
        .await
        .expect("flush stats before reopen");
    drop(proxy);
