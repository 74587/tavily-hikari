...
- `GET /api/admin/ha/status` additionally exposes `peerCount` and
  `syncDisabledReason=no_configured_peers` when `active_standby` has neither a configured remote
  peer nor a legacy `HA_SYNC_SOURCE_URL` pull source. This is diagnostic only: the service remains
  healthy and does not invent a peer or disable its local serving role.
