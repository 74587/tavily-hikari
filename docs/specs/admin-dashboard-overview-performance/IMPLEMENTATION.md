...
- Quota charge and the bounded recent-jobs section publish through immutable last-good patches when
  their signatures are the only changed overview inputs. Quota recovery remains keyset-bounded, and
  request-log, Alerts, rollup, and other summary freshness changes retain the complete rebuild path.
