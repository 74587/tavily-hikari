...
- 已生效权益叠加到账户有效 `monthlyLimit`，不改变 hourly、daily、hourly-any 额度。
...
- 价格固定为 `100 LDC = 1000 积分额度 / 自然月`。
- 用户选择的积分额度必须为 `1000` 的正整数倍；自然月数必须 `>= 1`。
...
- 当前月份的充值权益必须加入账户有效 `monthlyLimit`，`block_all` 生效时最终额度仍为 0。
...
- 用户打开 `/console`，在充值卡片中选择积分档位与自然月数。
...
- Given 用户本月有 `3000` 充值权益且基线/标签有效月额度为 `5000`
