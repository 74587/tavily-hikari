...
- Startup uses `schema_migrations(version,name,checksum,applied_at)` as the synchronous additive migration ledger. Existing production layouts are adopted only after a critical-object check; checksum drift or missing critical objects fails startup closed. Warm production startup skips registered DDL and runs only bounded semantic maintenance.
